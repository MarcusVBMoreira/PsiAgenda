<?php
// Icones do Lucide (mesma fonte visual do lucide-react usado na versao
// Next.js), embutidos como SVG estatico — sem dependencia de nenhuma
// biblioteca JS. Paths obtidos de lucide-static (MIT/ISC).
...
function icon(string $name, string $classes = 'h-4 w-4'): void
{
    $inner = ICONS[$name] ?? '';
    echo '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" '
        . 'fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" '
        . 'stroke-linejoin="round" class="' . h($classes) . '" aria-hidden="true">' . $inner . '</svg>';
}

/** Mesma coisa, mas retorna a string em vez de ecoar (uso dentro de expressoes). */
function iconHtml(string $name, string $classes = 'h-4 w-4'): string
{
    ob_start();
    icon($name, $classes);
    return ob_get_clean();
